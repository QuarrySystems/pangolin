...
describe("overlayCapabilities — agora-defined manifest rules", () => {
  it("uses array-union for agora-notifications.json at the root", async () => {
    const a: CapabilityBundle = {
      name: "a",
      files: { "agora-notifications.json": json(["slack", "email"]) },
    };
    const b: CapabilityBundle = {
      name: "b",
      files: { "agora-notifications.json": json(["email", "webhook"]) },
    };
    ...
    const merged = await readJson(
      join(workspaceDir, "agora-notifications.json"),
    );
    expect(merged).toEqual(["slack", "email", "webhook"]);
  });

  it("uses last-write-wins for agora-channel.json", async () => {
    const a: CapabilityBundle = {
      name: "a",
      files: { "agora-channel.json": json({ name: "old" }) },
    };
    const b: CapabilityBundle = {
      name: "b",
      files: { "agora-channel.json": json({ name: "new" }) },
    };
    ...
    expect(await readJson(join(workspaceDir, "agora-channel.json"))).toEqual({
      name: "new",
    });
  });

  it("uses last-write-wins for agora-setup.sh", async () => {
    const a: CapabilityBundle = {
      name: "a",
      files: { "agora-setup.sh": utf8("#!/bin/sh\necho first\n") },
    };
    const b: CapabilityBundle = {
      name: "b",
      files: { "agora-setup.sh": utf8("#!/bin/sh\necho second\n") },
    ...
    expect(await readUtf8(join(workspaceDir, "agora-setup.sh"))).toBe(
      "#!/bin/sh\necho second\n",
    );
  });

  it("matches agora-notifications.json under a nested directory", async () => {
    const a: CapabilityBundle = {
      name: "a",
      files: { "subdir/agora-notifications.json": json(["x"]) },
    };
    const b: CapabilityBundle = {
      name: "b",
      files: { "subdir/agora-notifications.json": json(["x", "y"]) },
    };
    ...
    const merged = await readJson(
      join(workspaceDir, "subdir/agora-notifications.json"),
    );
...
describe("overlayCapabilities — rule precedence", () => {
  it("agora-manifest rules take precedence over adapter reserved paths", async () => {
    // Adapter wants to claim agora-notifications.json as last-write-wins,
    // but the agora rule (array-union) must win.
    const adapter = makeAdapter({
      reservedPaths: ["agora-notifications.json"],
      mergeRules: {
        "agora-notifications.json": { strategy: "last-write-wins" },
      },
    });
    const a: CapabilityBundle = {
      name: "a",
      files: { "agora-notifications.json": json(["x"]) },
    };
    const b: CapabilityBundle = {
      name: "b",
      files: { "agora-notifications.json": json(["y"]) },
    };
    ...
    expect(
      await readJson(join(workspaceDir, "agora-notifications.json")),
    ).toEqual(["x", "y"]);
  });
});
...
describe("overlayCapabilities — type conflicts propagate", () => {
  ...
  it("propagates MergeTypeConflictError from array-union when input is not an array", async () => {
    // agora-notifications.json is forced array-union — feed it a non-array
    ...
    const b: CapabilityBundle = {
      name: "b",
      files: { "agora-notifications.json": json({ not: "array" }) },
    };
