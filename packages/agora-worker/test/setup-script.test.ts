...
describe("runSetupScriptIfPresent", () => {
  ...
  it("returns null when agora-setup.sh is absent", async () => {
  ...
  itPosix("returns SetupScriptResult with exitCode 0 on success", async () => {
    await writeFile(
      join(dir, "agora-setup.sh"),
      "#!/bin/bash\necho hello-stdout\necho hello-stderr 1>&2\nexit 0\n",
    );
    await chmod(join(dir, "agora-setup.sh"), 0o755);
    const res = await runSetupScriptIfPresent({
      workspaceDir: dir,
      env: {},
      timeoutSeconds: 5,
  ...
  itPosix("passes env to the spawned shell", async () => {
    await writeFile(
      join(dir, "agora-setup.sh"),
      '#!/bin/bash\necho "GOT=$AGORA_TEST_VAR"\nexit 0\n',
    );
    await chmod(join(dir, "agora-setup.sh"), 0o755);
    const res = await runSetupScriptIfPresent({
      workspaceDir: dir,
      env: { AGORA_TEST_VAR: "from-env", PATH: process.env.PATH ?? "" },
      timeoutSeconds: 5,
    });
    expect(res).not.toBeNull();
