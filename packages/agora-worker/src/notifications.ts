// agora-worker: notifications (§6.2 step 12 / §6.3).
//
...
//
//   1. `agora-notifications.json` inside the post-overlay workspace
...
// `signCallback` from agora-client (§7.3): hex HMAC-SHA256 over
 ...
 * workspace. Returns `[]` when `agora-notifications.json` is absent so the
 * caller can unconditionally merge with dispatch-level configs.
 */
export async function loadCapabilityNotifications(
  workspaceDir: string,
): Promise<NotificationConfig[]> {
  const path = join(workspaceDir, 'agora-notifications.json');
  try {
    await access(path);
  } catch {
    return [];
...
export async function fireNotifications(opts: {
  ...
  const headers = {
    ...
    'X-Agora-Dispatch-Id': opts.event.dispatchId,
