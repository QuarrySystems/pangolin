...
describe('FargateProvider', () => {
  it('RunTask carries the dispatchId tag, env entries, and digest-pinned image', async () => {
    ...
    expect(calls[0].tags).toContainEqual({ key: 'agora:dispatchId', value: 'd1' });
    ...
    try {
      ...
      const states = [
        ...
        { lastStatus: 'RUNNING' },
        {
          ...
          containers: [{ name: 'agora-worker', exitCode: 42 }],
        },
      ];
      let i = 0;
  ...
  it('cancel calls StopTask with agora.cancel reason', async () => {
    ...
    expect(captured!.reason).toBe('agora.cancel');
  });
});
