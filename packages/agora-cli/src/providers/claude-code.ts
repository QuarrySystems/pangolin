...
// description, tools, ...) + markdown body. The body becomes the agora
// subagent's `systemPrompt`; `model` carries through; `description` and
// `tools` have no agora equivalent and are dropped (tools = Claude Code
...
// `agora-setup.sh` is a single-slot `last-write-wins` file (overlay-engine
// AGORA_MANIFEST_RULES), so synthesizing one per capability would collide
 ...
 * same hash) are silently kept once — common when a sync upstream of agora
