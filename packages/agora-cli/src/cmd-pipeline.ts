// `agora pipeline` subcommand group — register / validate / list.
//
...
//   `(client.pipeline as any).list` is intentional: `AgoraClientPipelineAPI`
//   does not expose `list` yet (catalog enumeration is deferred in DAG 2).
//   Once `AgoraClientPipelineAPI` gains a `list()` method the cast can be
//   removed and this comment deleted.
//
// VALIDATE — storage-free design:
//   `validate` uses `registerPipeline` from `@quarry-systems/agora-client`
...
//   No client config file is needed. The AgoraClient namespace is set to a
...
/**
 * Build a stub AgoraClient that has enough shape to satisfy `registerPipeline`
...
export function attachPipelineCmd(program: Command, ctx: CliContext): void {
  ...
  pipe
    ...
    .action(async (file: string) => {
      ...
      // (`agora://<namespace>/pipeline/<id>@<hash>`).
      const ns = (client as { namespace?: string }).namespace ?? 'local';
      const pinnedUri = `agora://${ns}/pipeline/${ref!.id}@${ref!.contentHash}`;
  ...
  // import of @quarry-systems/agora-core (which is not a direct dep of agora-cli).
  ...
  pipe
    ...
    .action(async () => {
      const client = await ctx.getClient();
      // Type assertion: AgoraClientPipelineAPI.list is not yet part of the
