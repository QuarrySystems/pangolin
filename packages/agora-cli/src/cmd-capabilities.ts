// `agora capabilities` subcommand group.
//
...
//
// The command obtains the `AgoraClient` lazily via `ctx.getClient()` so the
// process does not pay the agora.config.{ts,js,mjs} resolution cost until a
