// `pangolin deploy --from <manifest>` — the manifest reconciler (§4.5).
//
...
// (`pangolin deploy --from ./pangolin.config.yaml` from the project root resolves
...
export function attachDeployCmd(program: Command, ctx: CliContext): void {
  program
    ...
    .requiredOption('--from <path>', 'path to pangolin-manifest.yaml')
    .action(async (opts: { from: string }) => {
      ...
      for (const env of manifest.envs ?? []) {
        const ref = await client.env.register({
          ...
          // the SDK's secret types from pangolin-core directly.
