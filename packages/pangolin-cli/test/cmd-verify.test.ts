...
async function buildEntries(runId: string): Promise<{ entries: AuditEntryRow[]; root: Uint8Array }> {
  ...
  const { canonEntry } = await import('@quarry-systems/pangolin-orchestrator/src/audit/canon.js');
  const { chainHash, merkleRoot, leavesFromEntryHashes } = await import('@quarry-systems/pangolin-orchestrator/src/audit/merkle.js');
...
describe('attachVerifyCmd', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'pangolin-cli-verify-test-'));
  });
