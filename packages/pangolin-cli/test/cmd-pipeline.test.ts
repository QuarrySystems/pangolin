// Tests for `pangolin pipeline` subcommand group.
//
...
//   to exercise validatePipelineSpec without needing a full PangolinClient.
...
describe('attachPipelineCmd', () => {
  ...
  it('register: reads JSON file and calls client.pipeline.register, prints ref with pinnedUri', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'pangolin-pipeline-'));
    ...
    await program.parseAsync(['node', 'pangolin', 'pipeline', 'register', specFile]);
    ...
    expect(printed.pinnedUri).toBe('pangolin://test-ns/pipeline/data.transform@sha256:abc123');
  });
  ...
  it('register: exits 1 when file does not exist', async () => {
    ...
    attachPipelineCmd(program, { getClient: async () => ({} as any) });
    await expect(
      program.parseAsync(['node', 'pangolin', 'pipeline', 'register', '/no/such/file.json']),
  ...
  it('register: exits 1 and prints all errors when client.pipeline.register throws', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'pangolin-pipeline-invalid-'));
    ...
    attachPipelineCmd(program, { getClient: async () => mockClient as any });
    await expect(
      program.parseAsync(['node', 'pangolin', 'pipeline', 'register', specFile]),
    ).rejects.toThrow(/exit:1/);
  ...
  it('validate: exits 0 and prints "OK" for a valid spec (no client constructed)', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'pangolin-pipeline-val-'));
    const specFile = join(dir, 'spec.json');
    await writeFile(specFile, JSON.stringify(VALID_SPEC), 'utf8');
    ...
    await program.parseAsync(['node', 'pangolin', 'pipeline', 'validate', specFile]);
  ...
  it('validate: exits 1 and prints all errors for an invalid spec (no client constructed)', async () => {
    ...
    attachPipelineCmd(program, { getClient });
    await expect(
      program.parseAsync(['node', 'pangolin', 'pipeline', 'validate', specFile]),
    ).rejects.toThrow(/exit:1/);
  ...
  it('validate: exits 1 when file does not exist (no client constructed)', async () => {
    ...
    attachPipelineCmd(program, { getClient });
    await expect(
      program.parseAsync(['node', 'pangolin', 'pipeline', 'validate', '/no/such/file.json']),
    ).rejects.toThrow(/exit:1/);
  ...
  it('list: calls client.pipeline.list and prints each entry on a tab-delimited line', async () => {
    ...
    const program = new Command();
    attachPipelineCmd(program, { getClient: async () => mockClient as any });
    await program.parseAsync(['node', 'pangolin', 'pipeline', 'list']);
