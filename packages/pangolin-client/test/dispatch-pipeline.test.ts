...
const PINNED_PIPELINE_URI = `pangolin://ns/pipeline/my-pipeline/${PIPELINE_HASH}`;
...
describe('dispatchWork — pipelineRef', () => {
  it('a pinned pipelineRef lands in bundleRefs.pipeline and resolved.pipelineRef', async () => {
    ...
    // bundleRefs.pipeline must be present in PANGOLIN_BUNDLE_REFS_JSON
    expect(capturedSpec.current).toBeDefined();
    const bundleRefs = JSON.parse(capturedSpec.current!.env.PANGOLIN_BUNDLE_REFS_JSON);
  ...
  it('an unpinned pipelineRef throws naming pipelineRef', async () => {
    ...
    const unpinnedUri = 'pangolin://ns/pipeline/my-pipeline'; // no contentHash segment
  ...
  it('a malformed pipelineRef throws (parsePangolinUri rejects it)', async () => {
    const storage = makeMemoryStorage();
    storage.seed('s', 'subagent', 'ns', 'sha256:s', { name: 's' });
    const { compute } = makeCompute();
    const client = new PangolinClient({
