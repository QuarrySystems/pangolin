...
beforeEach(() => {
  ...
  vi.spyOn(callbackHmac, 'mintCallbackHmac').mockResolvedValue({
    ref: 'store-ref://pangolin/callback-hmac/hmac-AbCdEf',
    ttlSeconds: 7500,
  });
});

const WORKER_IMAGE = 'pangolin/worker:digest';

describe('dispatchWork — TaskSpec.env', () => {
  it('populates PANGOLIN_DISPATCH_ID and PANGOLIN_BUNDLE_REFS_JSON', async () => {
    const storage = makeMemoryStorage();
    ...
    expect(spec.env.PANGOLIN_DISPATCH_ID).toBe(result.dispatchId);
    expect(spec.env.PANGOLIN_DISPATCH_ID).toMatch(/^[0-9a-f-]{36}$/);
    expect(spec.env.PANGOLIN_NAMESPACE).toBe('ns');
    expect(spec.env.PANGOLIN_BUNDLE_REFS_JSON).toBeTruthy();
    const bundleRefs = JSON.parse(spec.env.PANGOLIN_BUNDLE_REFS_JSON);
    expect(bundleRefs.subagent.uri).toBe('pangolin://ns/subagent/my-sub/sha256:subhash');
    ...
    expect(spec.env.PANGOLIN_RUNTIME_ADAPTER).toBe('claude-code');
    expect(spec.env.PANGOLIN_INPUT_JSON).toBe('{}');
  ...
  it('uses caller-supplied dispatchId when given', async () => {
    ...
    expect(runs[0].spec.env.PANGOLIN_DISPATCH_ID).toBe('caller-supplied-id');
  });

  it('serializes input JSON into PANGOLIN_INPUT_JSON', async () => {
    const storage = makeMemoryStorage();
    storage.seed('s', 'subagent', 'ns', 'sha256:h', { name: 's' });
    const { compute, runs } = makeCompute();
    const client = new PangolinClient({
    ...
    expect(JSON.parse(runs[0].spec.env.PANGOLIN_INPUT_JSON)).toEqual({ hello: 'world', n: 7 });
  });
});
...
describe('dispatchWork — ref resolution', () => {
  it('resolves subagent short name to a SubagentRef via storage', async () => {
    ...
    const bundleRefs = JSON.parse(runs[0].spec.env.PANGOLIN_BUNDLE_REFS_JSON);
...
describe('dispatchWork — secrets + callback', () => {
  it('stages per-dispatch inline secrets via the target store and emits PANGOLIN_SECRET_STORE_KIND + PANGOLIN_SECRET_STORE_DIR', async () => {
    const storage = makeMemoryStorage();
    storage.seed('s', 'subagent', 'ns', 'sha256:s', { name: 's' });
    const { store, staged } = makeStore({ name: 'local-file', dir: '/tmp/pangolin-secrets' });
    const { compute, runs } = makeCompute();
    const client = new PangolinClient({
      namespace: 'ns',
      compute: { default: compute },
    ...
    const spec = runs[0].spec;
    expect(spec.env.PANGOLIN_SECRET_STORE_KIND).toBe('local-file');
    expect(spec.env.PANGOLIN_SECRET_STORE_DIR).toBe('/tmp/pangolin-secrets');
  });

  it('emits PANGOLIN_SECRET_STORE_KIND but NOT PANGOLIN_SECRET_STORE_DIR when store has no dir', async () => {
    const storage = makeMemoryStorage();
    storage.seed('s', 'subagent', 'ns', 'sha256:s', { name: 's' });
    ...
    expect(runs[0].spec.env.PANGOLIN_SECRET_STORE_KIND).toBe('aws-secrets-manager');
    expect(runs[0].spec.env.PANGOLIN_SECRET_STORE_DIR).toBeUndefined();
  });
  ...
  it('passes per-dispatch secret refs to the worker via PANGOLIN_PER_DISPATCH_SECRET_REFS_JSON', async () => {
    const storage = makeMemoryStorage();
    storage.seed('s', 'subagent', 'ns', 'sha256:s', { name: 's' });
    storage.seed('shared', 'env', 'ns', 'sha256:e', {
      kind: 'env-bundle',
    ...
    const raw = runs[0].spec.env.PANGOLIN_PER_DISPATCH_SECRET_REFS_JSON;
  ...
  it('mints callback HMAC and injects PANGOLIN_CALLBACK_URL + PANGOLIN_CALLBACK_TOKEN_REF when work.callback set', async () => {
    const storage = makeMemoryStorage();
    storage.seed('s', 'subagent', 'ns', 'sha256:s', { name: 's' });
    const { store } = makeStore({ name: 'test-store' });
    const { compute, runs } = makeCompute();
    ...
    expect(runs[0].spec.env.PANGOLIN_CALLBACK_URL).toBe('https://example.com/cb');
    expect(runs[0].spec.env.PANGOLIN_CALLBACK_TOKEN_REF).toMatch(/hmac-AbCdEf/);
  });
  ...
  it('does not mint HMAC when no callback is configured', async () => {
    ...
    expect(runs[0].spec.env.PANGOLIN_CALLBACK_URL).toBeUndefined();
    expect(runs[0].spec.env.PANGOLIN_CALLBACK_TOKEN_REF).toBeUndefined();
  });
...
describe('dispatchWork — provider + telemetry + sink + record', () => {
  it('selects target provider, calls run + awaitExit, and writes the dispatch record', async () => {
    ...
    const recordUri = `pangolin://ns/dispatches/${result.dispatchId}/record.json`;
  ...
  it('cleans up per-dispatch staged secrets after awaitExit via store.cleanupByTag', async () => {
    ...
    expect(cleanupCalls[0].tagKey).toBe('pangolin:dispatchId');
