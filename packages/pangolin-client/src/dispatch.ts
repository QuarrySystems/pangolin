...
//      `PANGOLIN_*` env vars per §6.1 plus `PANGOLIN_RUNTIME_ADAPTER`, and
...
export interface InFlightDispatch {
  ...
  readonly resolved: {
    ...
    workerImage: string;
    inputRefs: Record<string, string>; // key -> already-pinned pangolin:// URI (Wave-C manifest)
...
export async function fireWork(
  ...
  for (const [envName, entry] of Object.entries(work.secrets ?? {})) {
    ...
    const { ref } = await store!.stage({
      ...
      tags: { 'pangolin:dispatchId': dispatchId },
    });
  ...
  // 6. Build TaskSpec.env — bundle-ref descriptors + the seven PANGOLIN_* vars
  //    from §6.1. `callback` injects two extras when configured.
  //
  // Resolve inputRefs BEFORE the container starts: parse each URI with
  // parsePangolinUri (rejects malformed URIs) and verify the URI is pinned
  ...
  const resolvedInputRefs: Array<{ key: string; uri: string; contentHash: string }> =
    Object.entries(work.inputRefs ?? {}).map(([key, uri]) => {
      const { contentHash } = parsePangolinUri(uri); // throws on malformed
      if (!contentHash) {
        throw new Error(
          `dispatch: inputRefs['${key}'] must be a pinned pangolin:// URI (missing content hash): ${uri}`,
        );
      }
  ...
  // parsePangolinUri throws on malformed; missing contentHash is an explicit error.
  let resolvedPipelineRef: { uri: string; contentHash: string } | undefined;
  if (work.pipelineRef !== undefined) {
    const { contentHash } = parsePangolinUri(work.pipelineRef); // throws on malformed
    if (!contentHash) {
      throw new Error(
        `dispatch: pipelineRef must be a pinned pangolin:// URI (missing content hash): ${work.pipelineRef}`,
      );
    }
  ...
  const envVars: Record<string, string> = {
    ...
    PANGOLIN_PER_DISPATCH_SECRET_REFS_JSON: JSON.stringify(perDispatchSecretRefs),
  };
  // Emit the store kind and (when set) the on-disk directory so the provider
  // can bind-mount it into the worker container.
  if (store) {
    envVars.PANGOLIN_SECRET_STORE_KIND = store.name;
    if (store.dir) {
      envVars.PANGOLIN_SECRET_STORE_DIR = store.dir;
    }
  }
  if (work.model !== undefined && work.model !== '') {
    envVars.PANGOLIN_MODEL = work.model;
  }
  if (work.callback) {
...
/**
 * Orchestrate a dispatch end-to-end against an `PangolinClient` (blocking).
