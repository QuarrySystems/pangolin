// Reads `agora-plugins.json` from a workspace (post-overlay) and invokes
...
export async function installPluginsFromManifest(
  ...
  const manifestPath = join(opts.workspaceDir, "agora-plugins.json");
  try {
  ...
  if (!Array.isArray(parsed)) {
    throw new Error(
      "agora-plugins.json must be a JSON array of plugin names",
    );
  }
