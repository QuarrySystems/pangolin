 ...
 * dispatch env. Read from `AGORA_CLAUDE_PERMISSION_MODE`:
 *
 ...
 * agora-needs-input-helper "denied → sentinel" pattern) is tracked as a
 * follow-up.
 */
function resolveBypassFlag(env: Record<string, string>): boolean {
  const raw = env.AGORA_CLAUDE_PERMISSION_MODE;
  if (raw === undefined || raw === '') return true;
  if (raw === 'bypass') return true;
  if (raw === 'strict') return false;
  // eslint-disable-next-line no-console
  console.warn(
    `claude-code adapter: unrecognized AGORA_CLAUDE_PERMISSION_MODE='${raw}', falling back to 'bypass'`,
  );
  return true;
}
