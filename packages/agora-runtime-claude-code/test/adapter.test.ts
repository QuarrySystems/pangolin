...
describe("ClaudeCodeRuntimeAdapter — static shape", () => {
  ...
  it("mergeRules['agora-plugins.json'] is array-union", () => {
    const a = new ClaudeCodeRuntimeAdapter();
    expect(a.mergeRules?.["agora-plugins.json"]).toEqual({
...
describe("ClaudeCodeRuntimeAdapter — invoke()", () => {
  ...
  it.skipIf(skipOnWindows)(
    ...
    async () => {
      ...
      // No `.agora/needs_input.json` in the fresh tmp workspace.
  ...
  it.skipIf(skipOnWindows)(
    "returns needsInputSentinelPath when .agora/needs_input.json exists",
    async () => {
      await writeFile(stubBin, "#!/bin/bash\nexit 0\n");
      await chmod(stubBin, 0o755);
