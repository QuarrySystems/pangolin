...
export const GUARANTEE_RANK: Record<Guarantee, number> = {
  detect: 0,
  'external-immutable': 1,
  witnessed: 2,
};
...
export interface Signer {
  sign(rootHash: Uint8Array): Promise<Signature>;
...
export interface AuthorizationContext {
  ...
  submittedAt?: string;
  at?: string; // decision-time stamp the caller supplies (keeps authorizers pure)
}

export interface Authorizer {
  authorize(ctx: AuthorizationContext): Promise<Authorization>;
}
...
export interface VerificationReport {
  ...
  timeTier: TimeTier;
  failure?: 'chain' | 'anchor-missing' | 'root-mismatch' | 'signature' | 'handoff' | 'manifest'; // first failing check (kept for back-compat). NOTE: no 'time' variant — time is informational.
  authzTier?: AuthzTier;
  checks: {
    chain: CheckResult;
    root: CheckResult;
