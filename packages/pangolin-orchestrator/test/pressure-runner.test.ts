...
function scriptedExecutor(
  script: Record<string, { failTimes?: number; durationMs?: number }>,
  obs: Obs,
): Executor {
  ...
  return {
    ...
    async fire(item: WorkItem) {
      ...
      inflight.set(hash, {
        id: item.id,
        completeAt: Date.now() + (script[item.id]?.durationMs ?? 10),
        startedAt: Date.now(),
      });
      obs.inflight.add(hash);
      obs.maxInflight = Math.max(obs.maxInflight, obs.inflight.size);
...
// Poll until `done()` or the budget elapses. The budget is a generous CEILING — the loop
// exits early the moment `done()` is true (typical completion is well under a second), so a
// large value only adds headroom on a loaded box (where a tight budget flaked these
// real-timer pressure scenarios) and never slows a passing run. Stays under vitest's 30s
// testTimeout.
async function driveUntil(done: () => boolean, ms = 15_000): Promise<void> {
...
describe('offload-runner pressure tests', () => {
  it('SCENARIO 1: fans out disjoint-lock work to concurrency while shared-lock work serializes', async () => {
    ...
    const edits = ['a', 'b', 'c'].map((k) => ({
      id: `e-${k}`,
      executor: 'scripted',
      inputs: {},
      depends_on: [] as string[],
      resourceLocks: [`file-${k}`],
    }));
    const shared = ['x', 'y'].map((k) => ({
      id: `e-${k}`,
      executor: 'scripted',
      inputs: {},
      depends_on: [] as string[],
      resourceLocks: ['shared'],
    }));
    const verify: WorkItem = {
      id: 'verify',
      executor: 'scripted',
      inputs: {},
      depends_on: [...edits, ...shared].map((e) => e.id),
      resourceLocks: [],
    };
    ...
    const script = Object.fromEntries(
      run.items.map((i) => [i.id, { durationMs: i.id === 'verify' ? 10 : 70 }]),
    ) as Record<string, { durationMs?: number; failTimes?: number }>;
    const orch = new PangolinOrchestrator({
      store,
      executors: { scripted: scriptedExecutor(script, obs) },
    ...
    const overlap = (a: string, b: string) =>
      iv(a) && iv(b) && iv(a).start < iv(b).end && iv(b).start < iv(a).end;
    ...
    expect(iv('verify').start).toBeGreaterThanOrEqual(
      Math.max(...[...edits, ...shared].map((e) => iv(e.id).end)) - 1,
    ); // verify ran after deps
  });
  ...
  it('SCENARIO 2: flaky item recovers, always-failing item exhausts attempts and cascades skip to its dependent, independent branch still completes', async () => {
    const root = tmp('pangolin-pt2-');
    const obs = newObs();
    const store = new SqliteRunStateStore(join(root, 'state.db'));
    const transport = new MailboxSubmissionTransport(new LocalDirMailbox(join(root, 'store')));
    const script = {
      flaky: { failTimes: 1, durationMs: 8 },
      doomed: { failTimes: 99, durationMs: 8 },
      indep: { durationMs: 8 },
    };
    const orch = new PangolinOrchestrator({
      store,
  ...
  it('SCENARIO 3: survives a crash + restart on the same DB and storage; recoverStranded re-dispatches the in-flight item and the run completes without re-running pre-crash completed work', async () => {
    ...
    const orch1 = new PangolinOrchestrator({
      store: store1,
      executors: { scripted: scriptedExecutor(script, obs1) },
      triggers: { manual: new ManualTrigger() },
    ...
    const orch2 = new PangolinOrchestrator({
      store: store2,
      executors: { scripted: scriptedExecutor(script, obs2) },
      triggers: { manual: new ManualTrigger() },
      queues: { default: { concurrency: 2 } },
    });
    ...
    await driveUntil(() => orch2.getStatus().every((s) => s.status === 'done'));
    ac2.abort();
    await p2;
