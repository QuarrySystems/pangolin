...
describe('contracts', () => {
  it('RUN_STATUSES covers the eight lifecycle states', () => {
...
it('a minimal executor satisfies the extended fire signature', async () => {
  ...
  const ex: Executor = {
    ...
    async fire(_item, ctx) {
      ...
      return { dispatchHash: 'h1', manifestRef: 'm1' };
    },
    async reconcile() {
      return { status: 'done', resultRef: 'r1' };
    },
  };
  const fired = await ex.fire(
