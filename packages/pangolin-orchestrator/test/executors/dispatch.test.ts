...
// ---------------------------------------------------------------------------
// Helper: build a wired PangolinClient + DispatchExecutor for a given compute
...
// ---------------------------------------------------------------------------
// Helper: build a wired PangolinClient + DispatchExecutor with an injected fake
...
// ---------------------------------------------------------------------------
function captureDispatchFire(client: PangolinClient): { work?: DispatchWork & ClientDispatchOpts } {
...
describe('DispatchExecutor', () => {
  ...
  it('fire writes a content-addressed manifest and returns its ref', async () => {
    ...
    // manifestRef should be a pinned pangolin:// URI
    expect(manifestRef).toMatch(/^pangolin:\/\/ns\/manifest\/[^/]+\/sha256:[0-9a-f]{64}$/);
  ...
  it('fire without ctx uses empty strings and still returns manifestRef', async () => {
    ...
    expect(manifestRef).toMatch(/^pangolin:\/\//);
  ...
  it('threads inputs.inputRefs into the dispatch work', async () => {
    ...
    const capturingCompute: import('@quarry-systems/pangolin-core').ComputeProvider = {
      name: 'capturing',
      async run(spec, ctx) {
    ...
    const inputRefs = { patch: 'pangolin://ns/artifact/d/sha256:' + 'a'.repeat(64) };
    const item: WorkItem = {
      ...baseItem,
  ...
  it('drops non-string values in inputs.inputRefs without failing the dispatch', async () => {
    ...
    const pinnedUri = 'pangolin://ns/artifact/d/sha256:' + 'b'.repeat(64);
    const item: WorkItem = {
      ...baseItem,
      inputs: {
        subagent: 's',
  ...
  it('threads inputs.pipeline into the dispatch work as pipelineRef', async () => {
    ...
    const pipelineUri = 'pangolin://ns/pipeline/my-pipe/sha256:' + 'c'.repeat(64);
    const item: WorkItem = {
      ...baseItem,
  ...
  it('fire with inputs.pipeline seals pipelineRef into the manifest', async () => {
    ...
    const pipelineUri = 'pangolin://ns/pipeline/my-pipe/sha256:' + 'd'.repeat(64);
    const item: WorkItem = {
      ...baseItem,
      inputs: { subagent: 's', pipeline: pipelineUri },
    };
