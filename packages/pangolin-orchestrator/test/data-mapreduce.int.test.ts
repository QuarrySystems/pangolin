...
/** Build a minimal PangolinClient for registration (no compute or targets needed). */
function makeClient(storage: LocalStorageProvider): PangolinClient {
...
async function setupFixtures(storageDir: string): Promise<TestFixtures> {
  ...
  const subagentUri = buildPangolinUri({
    namespace: NAMESPACE,
    type: 'subagent',
