import { PangolinOrchestrator, type PangolinOrchestratorOptions } from '../../src/orchestrator.js';
...
export function makeOrch(
  ...
  executor: Executor,
  extra?: Partial<PangolinOrchestratorOptions>,
): { orch: PangolinOrchestrator; anchor: LocalAnchor } {
 ...
 */
export async function driveUntilDone(orch: PangolinOrchestrator, maxTicks = 32, runId?: string): Promise<void> {
...
export async function driveUntil(orch: PangolinOrchestrator, pred: () => boolean, maxTicks = 32): Promise<void> {
  for (let i = 0; i < maxTicks; i++) {
    if (pred()) return;
    await orch.tick('default');
  }
