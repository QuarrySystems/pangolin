...
it('passes extraEnv into the container Env (spec.env wins on collision)', async () => {
  ...
  const p = new LocalDockerProvider({
    ...
    extraEnv: { AGORA_S3_ENDPOINT: 'http://host.docker.internal:9000', AGORA_NAMESPACE: 'from-extra' },
  });
  await p.run(
    { image: 'img:tag', command: [], dispatchId: 'd', env: { AGORA_NAMESPACE: 'from-spec' }, secretRefs: {} } as any,
    {} as any,
  );
  expect(captured.cfg.Env).toContain('AGORA_S3_ENDPOINT=http://host.docker.internal:9000');
  expect(captured.cfg.Env).toContain('AGORA_NAMESPACE=from-spec'); // spec.env wins
});
  ...
  it('merges multiple extraEnv keys into the container Env', async () => {
    ...
    const p = new LocalDockerProvider({
      ...
      extraEnv: {
        ...
        AGORA_S3_ENDPOINT: 'http://minio:9000',
      },
    });
    await p.run(
    ...
    expect(captured.cfg.Env).toContain('AGORA_S3_ENDPOINT=http://minio:9000');
  });
});
