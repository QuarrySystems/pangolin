// @quarry-systems/pangolin-mcp — stdio transport bootstrap.
//
...
// (`registerPangolinTools`), which owns the run-time tool catalog enforced by
...
import type { OperationsApi } from '@quarry-systems/pangolin-orchestrator';
import { registerPangolinTools } from './tools.js';

/**
 * Options accepted by `runServer`. Only `client` is required; `name` and
 * `version` default to `'pangolin-mcp'` and `'0.1.0'` respectively and are
...
export interface RunServerOpts {
  /** PangolinClient instance the tools wrap. */
  ...
  orch?: OperationsApi;
  /** Override the server name; defaults to `'pangolin-mcp'`. */
  name?: string;
...
/**
 * Construct the MCP `Server`, register the nine run-time Pangolin Scale tools, and
...
export async function runServer(opts: RunServerOpts): Promise<void> {
  const server = new Server(
    { name: opts.name ?? 'pangolin-mcp', version: opts.version ?? '0.1.0' },
    { capabilities: { tools: {} } },
  );
  registerPangolinTools(server, opts.client, opts.orch);
