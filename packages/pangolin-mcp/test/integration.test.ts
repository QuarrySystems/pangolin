// End-to-end wiring tests for `registerPangolinTools`.
//
...
//      `PANGOLIN_TOOL_NAMES` allowlist;
//   2. No `pangolin_*_register` / `pangolin_*_assign` deploy-time tools leak;
//   3. Each CallTool name dispatches to the matching `PangolinClient` method
...
import type { OperationsApi, OutboxRecord } from '@quarry-systems/pangolin-orchestrator';
...
/**
 * Build an in-memory fake `PangolinClient`. Each method is a `vi.fn()` so tests
...
function makeServerWith(fake: ReturnType<typeof makeFakeClient>): Server {
  ...
  // The fake client's structural shape matches PangolinClient at the call sites
  // exercised here; the cast is necessary because vi.fn() return types do
  // not carry the full pangolin-core type tree.
  registerPangolinTools(server, fake as never);
  return server;
}
...
function makeServerWithOrch(fake: ReturnType<typeof makeFakeClient>, fakeOrch: OperationsApi): Server {
  ...
  registerPangolinTools(server, fake as never, fakeOrch);
  return server;
}
...
describe('pangolin-mcp registerPangolinTools — ListTools', () => {
  it('registers exactly nine tools matching PANGOLIN_TOOL_NAMES', async () => {
    const server = makeServerWith(makeFakeClient());
    const tools = await listTools(server);
    expect(tools.map((t) => t.name).sort()).toEqual([...PANGOLIN_TOOL_NAMES].sort());
    expect(tools).toHaveLength(9);
  });

  it('exposes no tool matching pangolin_*_register or pangolin_*_assign', async () => {
    const server = makeServerWith(makeFakeClient());
    const tools = await listTools(server);
...
describe('pangolin-mcp registerPangolinTools — CallTool dispatch', () => {
  it('pangolin_dispatch invokes client.dispatch with the supplied args and wraps the result', async () => {
    ...
    const res = await callTool(server, 'pangolin_dispatch', args);
  ...
  it('pangolin_dispatch_describe invokes client.dispatch.describe(dispatchId)', async () => {
    const fake = makeFakeClient();
    const server = makeServerWith(fake);
    const res = await callTool(server, 'pangolin_dispatch_describe', { dispatchId: 'abc' });
  ...
  it('pangolin_dispatch_cancel invokes client.dispatch.cancel(dispatchId)', async () => {
    const fake = makeFakeClient();
    const server = makeServerWith(fake);
    const res = await callTool(server, 'pangolin_dispatch_cancel', { dispatchId: 'xyz' });
  ...
  it('pangolin_capabilities_list invokes client.capabilities.list()', async () => {
    const fake = makeFakeClient();
    const server = makeServerWith(fake);
    const res = await callTool(server, 'pangolin_capabilities_list');
  ...
  it('pangolin_subagents_list invokes client.subagent.list()', async () => {
    const fake = makeFakeClient();
    const server = makeServerWith(fake);
    const res = await callTool(server, 'pangolin_subagents_list');
  ...
  it('pangolin_envs_list invokes client.env.list()', async () => {
    const fake = makeFakeClient();
    const server = makeServerWith(fake);
    const res = await callTool(server, 'pangolin_envs_list');
...
describe('pangolin-mcp registerPangolinTools — catalog leakage guard', () => {
...
describe('pangolin-mcp registerPangolinTools — error path', () => {
  it('unknown tool name returns isError: true rather than throwing', async () => {
    const fake = makeFakeClient();
    const server = makeServerWith(fake);
    // Must NOT throw.
    const res = await callTool(server, 'pangolin_nonexistent_tool');
  ...
  it('client method that throws becomes an isError response (no stack leak)', async () => {
    ...
    const server = makeServerWith(fake);
    const res = await callTool(server, 'pangolin_dispatch', {
      target: 't',
      subagent: 's',
      workerImage: 'i',
    });
  ...
  it('pangolin_dispatch_describe with missing dispatchId returns isError, does not throw', async () => {
    const fake = makeFakeClient();
    const server = makeServerWith(fake);
    const res = await callTool(server, 'pangolin_dispatch_describe', {});
    expect(res.isError).toBe(true);
...
describe('pangolin-mcp registerPangolinTools — orchestrator tools (not configured)', () => {
  it('pangolin_orchestrator_submit returns isError when orch not provided', async () => {
    const fake = makeFakeClient();
    const server = makeServerWith(fake);
    const res = await callTool(server, 'pangolin_orchestrator_submit', { plan: { id: 'r1', queue: 'q', items: [] } });
    expect(res.isError).toBe(true);
    expect(res.content[0].text).toContain('not configured');
  });

  it('pangolin_orchestrator_status returns isError when orch not provided', async () => {
    const fake = makeFakeClient();
    const server = makeServerWith(fake);
    const res = await callTool(server, 'pangolin_orchestrator_status', { runId: 'r1' });
    expect(res.isError).toBe(true);
    expect(res.content[0].text).toContain('not configured');
  });

  it('pangolin_orchestrator_watch returns isError when orch not provided', async () => {
    const fake = makeFakeClient();
    const server = makeServerWith(fake);
    const res = await callTool(server, 'pangolin_orchestrator_watch', { runId: 'r1' });
    expect(res.isError).toBe(true);
    expect(res.content[0].text).toContain('not configured');
  });
});

describe('pangolin-mcp registerPangolinTools — orchestrator tools (configured)', () => {
  it('pangolin_orchestrator_submit calls orch.submit with plan and default actor', async () => {
    const fake = makeFakeClient();
    const fakeOrch = makeFakeOrch();
    const server = makeServerWithOrch(fake, fakeOrch);
    const plan = { id: 'run-1', queue: 'q', items: [] };
    const res = await callTool(server, 'pangolin_orchestrator_submit', { plan });
  ...
  it('pangolin_orchestrator_submit uses provided actor', async () => {
    const fake = makeFakeClient();
    const fakeOrch = makeFakeOrch();
    const server = makeServerWithOrch(fake, fakeOrch);
    const plan = { id: 'run-2', queue: 'q', items: [] };
    await callTool(server, 'pangolin_orchestrator_submit', { plan, actor: 'agent:custom' });

    expect(fakeOrch.submit).toHaveBeenCalledWith(plan, 'agent:custom');
  });

  it('pangolin_orchestrator_status calls orch.status with runId', async () => {
    const fake = makeFakeClient();
    const fakeOrch = makeFakeOrch();
    const server = makeServerWithOrch(fake, fakeOrch);
    const res = await callTool(server, 'pangolin_orchestrator_status', { runId: 'run-1' });
  ...
  it('pangolin_orchestrator_status returns isError when runId missing', async () => {
    const fake = makeFakeClient();
    const fakeOrch = makeFakeOrch();
    const server = makeServerWithOrch(fake, fakeOrch);
    const res = await callTool(server, 'pangolin_orchestrator_status', {});
    expect(res.isError).toBe(true);
    expect(res.content[0].text).toMatch(/runId/);
  });

  it('pangolin_orchestrator_watch calls orch.watch and returns last record', async () => {
    const fake = makeFakeClient();
    const fakeOrch = makeFakeOrch();
    const server = makeServerWithOrch(fake, fakeOrch);
    const res = await callTool(server, 'pangolin_orchestrator_watch', { runId: 'run-1', timeoutMs: 5000 });
  ...
  it('pangolin_orchestrator_watch falls back to status when watch yields nothing', async () => {
    const fake = makeFakeClient();
    ...
    const server = makeServerWithOrch(fake, fakeOrch);
    const res = await callTool(server, 'pangolin_orchestrator_watch', { runId: 'run-1', timeoutMs: 100 });
