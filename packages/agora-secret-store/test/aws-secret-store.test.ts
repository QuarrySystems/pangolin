...
describe("AwsSecretStore", () => {
  describe("stage", () => {
    it("creates a secret and returns its ARN as the ref plus the ttl", async () => {
      ...
      const staged = await store.stage({
        name: "agora/inline/d-1/FOO",
        value: "s3cr3t",
        ttlSeconds: 7500,
        tags: { "agora:dispatchId": "d-1" },
      });
      ...
      expect(create!.input.Name).toBe("agora/inline/d-1/FOO");
      ...
      expect(tags).toContainEqual({ Key: "agora:dispatchId", Value: "d-1" });
      expect(tags).toContainEqual({ Key: "agora:ttlSeconds", Value: "7500" });
    });
  ...
  describe("cleanupByTag", () => {
    it("lists secrets by tag filter and force-deletes each, across pages", async () => {
      ...
      await store.cleanupByTag("agora:dispatchId", "d-1");
