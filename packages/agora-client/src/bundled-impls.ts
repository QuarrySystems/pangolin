...
/** Canonical DispatchResult.failure reasons (mirrors the union in agora-core). */
...
export class StdoutResultSink implements ResultSink {
  ...
  async collect(
    ...
    // Per agora-worker (entrypoint.ts step 13), a valid sentinel logs a JSON
 ...
 * event. The worker (agora-worker entrypoint.ts step 13) logs a JSON line of
