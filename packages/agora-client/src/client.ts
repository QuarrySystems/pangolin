// `AgoraClient` — caller-side SDK entry point (§7).
//
...
/**
 * Constructor options for `AgoraClient`. All fields except `namespace`,
...
export class AgoraClient {
  ...
  constructor(opts: AgoraClientOptions) {
    if (!opts.namespace) {
      throw new Error('AgoraClient: namespace is required');
    }
    if (!opts.storage) {
    ...
    for (const targetName of Object.keys(targets)) {
      ...
      if (!opts.compute[t.compute]) {
        throw new Error(
          `AgoraClient: target ${targetName} references unknown compute ${t.compute}`,
        );
      }
    ...
    if (maxDays > MAX_RETENTION_DAYS) {
      throw new Error(
        `AgoraClient: dispatchRetention.maxDays exceeds 7-year cap (${MAX_RETENTION_DAYS} days)`,
      );
    }
    if (defaultDays > maxDays) {
      throw new Error(
        `AgoraClient: dispatchRetention.defaultDays (${defaultDays}) exceeds maxDays (${maxDays})`,
      );
    }
