...
import { CapabilityTooLargeError, CredentialsInEnvError } from '@quarry-systems/agora-core';
...
describe('registerCapability', () => {
  ...
  it('writes the bundle blob to storage at the pinned URI', async () => {
    ...
    const pinnedUri = `agora://ns/capability/web-search/${ref.contentHash}`;
  ...
  it('changed content produces a new entry with a new contentHash and a new pinned write', async () => {
    ...
    expect(storage.blobs.has(`agora://ns/capability/web-search/${first.contentHash}`)).toBe(true);
    expect(storage.blobs.has(`agora://ns/capability/web-search/${second.contentHash}`)).toBe(true);
  });
