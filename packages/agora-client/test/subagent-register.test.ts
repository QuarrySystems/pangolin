...
function makeMemoryStorage(): StorageProvider & {
  ...
  return {
    ...
    async put(uri: string, contents: Uint8Array) {
      // Pinned uri shape: agora://ns/type/name/<contentHash>
...
describe('registerSubagent', () => {
  ...
  it('writes the subagent definition to storage at the pinned URI', async () => {
    ...
    const pinnedUri = `agora://ns/subagent/reviewer/${handle.contentHash}`;
  ...
  it('resolves short-name capability refs against storage.resolveLatest', async () => {
    ...
    await storage.put('agora://ns/capability/web-search/sha256:capabilityhash', new TextEncoder().encode('{}'));
    const client = makeClient(storage);
    const handle = await registerSubagent(client, {
      name: 'reviewer',
      systemPrompt: 'hi',
  ...
  it('content hash depends on the resolved capability hashes', async () => {
    const storage = makeMemoryStorage();
    await storage.put('agora://ns/capability/cap-a/sha256:caphasha', new TextEncoder().encode('{}'));
    await storage.put('agora://ns/capability/cap-b/sha256:caphashb', new TextEncoder().encode('{}'));
    const client = makeClient(storage);
  ...
  it('assign() creates a new version with a different content hash; both versions remain in storage', async () => {
    ...
    const originalPinned = `agora://ns/subagent/reviewer/${original.contentHash}`;
    const evolvedPinned = `agora://ns/subagent/reviewer/${evolved.contentHash}`;
