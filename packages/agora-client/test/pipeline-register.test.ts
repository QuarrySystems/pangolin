...
import type { StorageProvider, PipelineSpec } from '@quarry-systems/agora-core';
...
describe('registerPipeline', () => {
  ...
  it('writes the pipeline definition to storage at the pinned URI', async () => {
    ...
    const ref = await registerPipeline(client, validSpec);
    const pinnedUri = `agora://ns/pipeline/data.transform/${ref.contentHash}`;
  ...
  it('different spec same id produces a NEW pinned version; both are retrievable', async () => {
    ...
    const pinnedV1 = `agora://ns/pipeline/data.transform/${refV1.contentHash}`;
    const pinnedV2 = `agora://ns/pipeline/data.transform/${refV2.contentHash}`;
  ...
  it('client.pipeline.register is reachable via the namespace API', async () => {
    ...
    const { AgoraClient: _ } = await import('../src/index.js');
