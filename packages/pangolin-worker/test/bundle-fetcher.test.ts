...
function subRef(storage: FakeStorage): BundleRefs["subagent"] {
  const uri = "pangolin://ns/subagent/s/sha256:s";
...
describe("fetchBundles", () => {
  it("fetches and verifies a subagent JSON bundle", async () => {
    ...
    const uri = "pangolin://ns/subagent/alpha/sha256:1";
  ...
  it("fetches capability bundles in declared order and verifies packed-bytes hash", async () => {
    ...
    const storage = new FakeStorage();
    const uriA = "pangolin://ns/capability/cap-a/sha256:a";
    const uriB = "pangolin://ns/capability/cap-b/sha256:b";
  ...
  it("throws IntegrityMismatchError when a capability hash does not match", async () => {
    const storage = new FakeStorage();
    storage.set(
      "pangolin://ns/subagent/s/sha256:s",
      asBytes({}),
    );
    storage.set(
      "pangolin://ns/capability/c/sha256:c",
    ...
    const refs: BundleRefs = {
      ...
      capabilities: [
        {
          uri: "pangolin://ns/capability/c/sha256:c",
  ...
  it("fetches and verifies env JSON bundles", async () => {
    ...
    const storage = new FakeStorage();
    storage.set("pangolin://ns/subagent/s/sha256:s", asBytes({}));
    storage.set("pangolin://ns/env/e/sha256:e", asBytes(envDef));
    const refs: BundleRefs = {
      subagent: {
        uri: "pangolin://ns/subagent/s/sha256:s",
        contentHash: computeContentHash({}),
      ...
      env: [
        {
          uri: "pangolin://ns/env/e/sha256:e",
  ...
  it("throws IntegrityMismatchError when env hash does not match", async () => {
    const storage = new FakeStorage();
    storage.set("pangolin://ns/subagent/s/sha256:s", asBytes({}));
    storage.set("pangolin://ns/env/e/sha256:e", asBytes({ tampered: true }));
    const refs: BundleRefs = {
      subagent: {
        uri: "pangolin://ns/subagent/s/sha256:s",
        contentHash: computeContentHash({}),
  ...
  it("returns subagent + capabilities + envs together when all hashes match", async () => {
    ...
    const envDef = { e: 1 };
    const storage = new FakeStorage();
    storage.set("pangolin://ns/subagent/alpha/sha256:s", asBytes(subagentDef));
    storage.set("pangolin://ns/capability/cap/sha256:c", capBytes);
    storage.set("pangolin://ns/env/env1/sha256:e", asBytes(envDef));
    const refs: BundleRefs = {
      subagent: {
        uri: "pangolin://ns/subagent/alpha/sha256:s",
        contentHash: computeContentHash(subagentDef),
      },
      capabilities: [
        {
          uri: "pangolin://ns/capability/cap/sha256:c",
          contentHash: computeContentHash(capBytes),
        },
      ],
      env: [
        {
          uri: "pangolin://ns/env/env1/sha256:e",
          contentHash: computeContentHash(envDef),
        },
      ],
    };
  ...
  it("fetches and raw-bytes-verifies input refs", async () => {
    ...
    const refs: BundleRefs = {
      ...
      inputs: [
        {
          key: "patch",
          uri: "pangolin://ns/artifact/d/sha256:p",
          contentHash: computeContentHash(bytes),
        },
      ],
    };
    storage.set("pangolin://ns/artifact/d/sha256:p", bytes);
  ...
  it("fetches and object-hash-verifies the pipeline bundle; tampered bytes throw", async () => {
    ...
    const storage = new FakeStorage();
    const uri = "pangolin://ns/pipeline/p/sha256:pp";

    // Store canonicalJsonString(spec) bytes at the pinned URI
    const { canonicalJsonString } = await import("@quarry-systems/pangolin-core");
...
describe("constructStorageProvider", () => {
  ...
  it("constructs a LocalStorageProvider for file:// URIs", async () => {
    const provider = await constructStorageProvider("file:///tmp/pangolin-store");
    expect(provider.name).toBe("local-fs");
  });

  it("constructs a LocalStorageProvider for bare absolute paths", async () => {
    const provider = await constructStorageProvider("/tmp/pangolin-store");
    expect(provider.name).toBe("local-fs");
  });
