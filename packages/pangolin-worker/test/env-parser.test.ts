...
describe("parseWorkerEnv", () => {
  ...
  describe("secret store kind", () => {
    ...
    it("accepts explicit aws-secrets-manager and round-trips correctly", () => {
      const cfg = parseWorkerEnv({ ...baseEnv(), PANGOLIN_SECRET_STORE_KIND: "aws-secrets-manager" });
      expect(cfg.secretStoreKind).toBe("aws-secrets-manager");
    });
  ...
  describe("bundle refs inputs", () => {
    it("parses bundleRefs.inputs when present", () => {
      ...
      env.PANGOLIN_BUNDLE_REFS_JSON = JSON.stringify({
        ...
        capabilities: [],
        env: [],
        inputs: [{ key: "patch", uri: "pangolin://ns/artifact/d/sha256:bb", contentHash: "sha256:bb" }],
      });
