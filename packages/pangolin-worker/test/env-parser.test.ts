...
function baseEnv(): NodeJS.ProcessEnv {
  return {
    PANGOLIN_DISPATCH_ID: 'd-123',
    PANGOLIN_NAMESPACE: 'ns-1',
    PANGOLIN_STORAGE_URI: 's3://bucket/prefix',
    PANGOLIN_BUNDLE_REFS_JSON: validBundleRefs,
  };
}

describe('parseWorkerEnv', () => {
  describe('required vars', () => {
    it('throws when PANGOLIN_DISPATCH_ID is missing', () => {
      expect(() => parseWorkerEnv({})).toThrow(/PANGOLIN_DISPATCH_ID/);
    });

    it('throws when PANGOLIN_NAMESPACE is missing', () => {
      const env = baseEnv();
      delete env.PANGOLIN_NAMESPACE;
      expect(() => parseWorkerEnv(env)).toThrow(/PANGOLIN_NAMESPACE/);
    });

    it('throws when PANGOLIN_STORAGE_URI is missing', () => {
      const env = baseEnv();
      delete env.PANGOLIN_STORAGE_URI;
      expect(() => parseWorkerEnv(env)).toThrow(/PANGOLIN_STORAGE_URI/);
    });

    it('throws when PANGOLIN_BUNDLE_REFS_JSON is missing', () => {
      const env = baseEnv();
      delete env.PANGOLIN_BUNDLE_REFS_JSON;
      expect(() => parseWorkerEnv(env)).toThrow(/PANGOLIN_BUNDLE_REFS_JSON/);
    });
  });
  ...
  describe('per-dispatch secret refs', () => {
    ...
    it('parses PANGOLIN_PER_DISPATCH_SECRET_REFS_JSON into envName→ref map', () => {
      const env = baseEnv();
      env.PANGOLIN_PER_DISPATCH_SECRET_REFS_JSON = JSON.stringify({
    ...
    it('throws when PANGOLIN_PER_DISPATCH_SECRET_REFS_JSON is malformed JSON', () => {
      const env = baseEnv();
      env.PANGOLIN_PER_DISPATCH_SECRET_REFS_JSON = '{not-json';
      expect(() => parseWorkerEnv(env)).toThrow(
        /PANGOLIN_PER_DISPATCH_SECRET_REFS_JSON.*not valid JSON/,
      );
    });
  });

  describe('secret store dir', () => {
    it('leaves secretStoreDir undefined when PANGOLIN_SECRET_STORE_DIR is unset', () => {
      expect(parseWorkerEnv(baseEnv()).secretStoreDir).toBeUndefined();
    });

    it('parses PANGOLIN_SECRET_STORE_DIR into secretStoreDir', () => {
      const env = baseEnv();
      env.PANGOLIN_SECRET_STORE_DIR = '/pangolin/secrets';
      expect(parseWorkerEnv(env).secretStoreDir).toBe('/pangolin/secrets');
    });
  });

  describe('bundle refs validation', () => {
    it('throws when PANGOLIN_BUNDLE_REFS_JSON is malformed JSON', () => {
      const env = baseEnv();
      env.PANGOLIN_BUNDLE_REFS_JSON = '{not-json';
      expect(() => parseWorkerEnv(env)).toThrow(/PANGOLIN_BUNDLE_REFS_JSON.*not valid JSON/);
    });

    it('throws when bundle refs is missing subagent field', () => {
      const env = baseEnv();
      env.PANGOLIN_BUNDLE_REFS_JSON = JSON.stringify({ capabilities: [], env: [] });
      expect(() => parseWorkerEnv(env)).toThrow(/subagent/);
    });

    it('throws when bundle refs capabilities is not an array', () => {
      const env = baseEnv();
      env.PANGOLIN_BUNDLE_REFS_JSON = JSON.stringify({
  ...
  describe('input JSON', () => {
    it('defaults inputJson to empty object when PANGOLIN_INPUT_JSON is absent', () => {
      const cfg = parseWorkerEnv(baseEnv());
      expect(cfg.inputJson).toEqual({});
    });

    it('parses PANGOLIN_INPUT_JSON when present', () => {
      const env = baseEnv();
      env.PANGOLIN_INPUT_JSON = JSON.stringify({ foo: 1, bar: 'x' });
      const cfg = parseWorkerEnv(env);
  ...
  describe('callback pairing', () => {
    it('throws when PANGOLIN_CALLBACK_URL is set without PANGOLIN_CALLBACK_TOKEN_REF', () => {
      const env = baseEnv();
  ...
  describe('defaults', () => {
    ...
    it('honours PANGOLIN_RUNTIME_ADAPTER override', () => {
      const env = baseEnv();
      env.PANGOLIN_RUNTIME_ADAPTER = 'codex';
    ...
    it('parses PANGOLIN_SETUP_TIMEOUT_SECONDS as a number', () => {
      const env = baseEnv();
      env.PANGOLIN_SETUP_TIMEOUT_SECONDS = '300';
      expect(parseWorkerEnv(env).setupTimeoutSeconds).toBe(300);
    });

    it('throws when PANGOLIN_SETUP_TIMEOUT_SECONDS is not a valid non-negative integer', () => {
      const env = baseEnv();
      env.PANGOLIN_SETUP_TIMEOUT_SECONDS = 'abc';
      expect(() => parseWorkerEnv(env)).toThrow(
        /PANGOLIN_SETUP_TIMEOUT_SECONDS must be a non-negative integer/,
      );
    });

    // KNOWN-ISSUES 9: these two are emitted by pangolin-client into the TASK
    // env, which is the worker's own process.env. They are parsed HERE rather
    // than read by the adapter from its runtime env, because filterRuntimeEnv
    // is default-deny and strips every PANGOLIN_* var — an adapter-side read
    // would silently never see them.
    it('parses PANGOLIN_AGENT_TIMEOUT_SECONDS as a number', () => {
      const env = baseEnv();
      env.PANGOLIN_AGENT_TIMEOUT_SECONDS = '1800';
      expect(parseWorkerEnv(env).agentTimeoutSeconds).toBe(1800);
    });

    it("defaults agentTimeoutSeconds to 7200, matching the client's floor", () => {
      expect(parseWorkerEnv(baseEnv()).agentTimeoutSeconds).toBe(7200);
    });

    it('parses PANGOLIN_PLUGIN_INSTALL_TIMEOUT_SECONDS as a number', () => {
      const env = baseEnv();
      env.PANGOLIN_PLUGIN_INSTALL_TIMEOUT_SECONDS = '90';
      expect(parseWorkerEnv(env).pluginInstallTimeoutSeconds).toBe(90);
    });

    it('defaults pluginInstallTimeoutSeconds to 300', () => {
      expect(parseWorkerEnv(baseEnv()).pluginInstallTimeoutSeconds).toBe(300);
    });

    it('throws when PANGOLIN_AGENT_TIMEOUT_SECONDS is not a valid integer', () => {
      const env = baseEnv();
      env.PANGOLIN_AGENT_TIMEOUT_SECONDS = 'soon';
      expect(() => parseWorkerEnv(env)).toThrow(/PANGOLIN_AGENT_TIMEOUT_SECONDS/);
    });
    ...
    it('disableNeedsInputHelper is true only when PANGOLIN_DISABLE_NEEDS_INPUT_HELPER=true', () => {
      const env = baseEnv();
    ...
    it('disableNeedsInputHelper stays false for other truthy-ish strings', () => {
      const env = baseEnv();
      env.PANGOLIN_DISABLE_NEEDS_INPUT_HELPER = '1';
  ...
  describe('secret store kind', () => {
    ...
    it('reads local-file from PANGOLIN_SECRET_STORE_KIND', () => {
      const cfg = parseWorkerEnv({ ...baseEnv(), PANGOLIN_SECRET_STORE_KIND: 'local-file' });
    ...
    it('throws with a clear message when PANGOLIN_SECRET_STORE_KIND is unrecognized', () => {
      expect(() =>
        parseWorkerEnv({ ...baseEnv(), PANGOLIN_SECRET_STORE_KIND: 'typo-value' }),
      ).toThrow(/PANGOLIN_SECRET_STORE_KIND/);
    });
  });
  ...
  describe('bundle refs pipeline', () => {
    ...
    it('parses bundleRefs.pipeline when present', () => {
      ...
      env.PANGOLIN_BUNDLE_REFS_JSON = JSON.stringify({
        ...
        capabilities: [],
        env: [],
        pipeline: { uri: 'pangolin://ns/pipeline/p/sha256:pp', contentHash: 'sha256:pp' },
      });
      expect(parseWorkerEnv(env).bundleRefs.pipeline).toEqual({
        uri: 'pangolin://ns/pipeline/p/sha256:pp',
    ...
    it('throws when pipeline is not an object', () => {
      ...
      expect(() => parseWorkerEnv(env)).toThrow(/PANGOLIN_BUNDLE_REFS_JSON.*pipeline/);
    });
    ...
    it('throws when pipeline is missing contentHash', () => {
      ...
      env.PANGOLIN_BUNDLE_REFS_JSON = JSON.stringify({
        ...
        capabilities: [],
        env: [],
        pipeline: { uri: 'pangolin://ns/pipeline/p/sha256:pp' },
      });
      expect(() => parseWorkerEnv(env)).toThrow(/PANGOLIN_BUNDLE_REFS_JSON.*pipeline/);
    });
    ...
    it('throws when pipeline contentHash is not a string', () => {
      ...
      env.PANGOLIN_BUNDLE_REFS_JSON = JSON.stringify({
        ...
        capabilities: [],
        env: [],
        pipeline: { uri: 'pangolin://ns/pipeline/p/sha256:pp', contentHash: 123 },
      });
      expect(() => parseWorkerEnv(env)).toThrow(/PANGOLIN_BUNDLE_REFS_JSON.*pipeline/);
    });
  });
  ...
  describe('bundle refs inputs', () => {
    it('parses bundleRefs.inputs when present', () => {
      ...
      env.PANGOLIN_BUNDLE_REFS_JSON = JSON.stringify({
        ...
        env: [],
        inputs: [
          { key: 'patch', uri: 'pangolin://ns/artifact/d/sha256:bb', contentHash: 'sha256:bb' },
        ],
      });
    ...
    it('throws when inputs is not an array', () => {
      ...
      expect(() => parseWorkerEnv(env)).toThrow(/PANGOLIN_BUNDLE_REFS_JSON.*inputs/);
    });
    ...
    it('throws when inputs entry is missing key', () => {
      ...
      env.PANGOLIN_BUNDLE_REFS_JSON = JSON.stringify({
        ...
        capabilities: [],
        env: [],
        inputs: [{ uri: 'pangolin://ns/artifact/d/sha256:bb', contentHash: 'sha256:bb' }],
      });
      expect(() => parseWorkerEnv(env)).toThrow(/PANGOLIN_BUNDLE_REFS_JSON.*inputs/);
    });
    ...
    it('throws when inputs entry is missing contentHash', () => {
      ...
      env.PANGOLIN_BUNDLE_REFS_JSON = JSON.stringify({
        ...
        capabilities: [],
        env: [],
        inputs: [{ key: 'patch', uri: 'pangolin://ns/artifact/d/sha256:bb' }],
      });
      expect(() => parseWorkerEnv(env)).toThrow(/PANGOLIN_BUNDLE_REFS_JSON.*inputs/);
    });
    ...
    it('throws when inputs entry fields are not strings', () => {
      ...
      env.PANGOLIN_BUNDLE_REFS_JSON = JSON.stringify({
        ...
        capabilities: [],
        env: [],
        inputs: [{ key: 123, uri: 'pangolin://ns/artifact/d/sha256:bb', contentHash: 'sha256:bb' }],
      });
      expect(() => parseWorkerEnv(env)).toThrow(/PANGOLIN_BUNDLE_REFS_JSON.*inputs/);
    });
    ...
    it('supports multiple inputs entries', () => {
      ...
      env.PANGOLIN_BUNDLE_REFS_JSON = JSON.stringify({
        ...
        inputs: [
          { key: 'patch', uri: 'pangolin://ns/artifact/d/sha256:bb', contentHash: 'sha256:bb' },
          { key: 'config', uri: 'pangolin://ns/artifact/d/sha256:cc', contentHash: 'sha256:cc' },
        ],
      });
  ...
  describe('model', () => {
    it('parses PANGOLIN_MODEL when present and omits it when absent or empty', () => {
      expect(parseWorkerEnv({ ...baseEnv(), PANGOLIN_MODEL: 'max' }).model).toBe('max');
