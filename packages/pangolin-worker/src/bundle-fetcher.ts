// pangolin-worker: bundle fetcher
//
// Fetches the subagent / capability / env bundles advertised in
// PANGOLIN_BUNDLE_REFS_JSON from a StorageProvider and verifies each against
// its declared content hash. Hash mismatch -> IntegrityMismatchError.
//
// Implements §6.2 steps 1-2 and §7.2 of the pangolin-core spec: the worker
...
/**
 * Construct the appropriate StorageProvider for the given PANGOLIN_STORAGE_URI.
 *
...
export async function constructStorageProvider(
  ...
  if (storageUri.startsWith("s3://")) {
    let S3StorageProvider: typeof import("@quarry-systems/pangolin-storage-s3").S3StorageProvider;
    try {
      ...
      throw new Error(
        `pangolin-worker: failed to load S3StorageProvider: ${(err as Error).message}`,
      );
    }
  ...
  if (storageUri.startsWith("file://") || storageUri.startsWith("/")) {
    let LocalStorageProvider: typeof import("@quarry-systems/pangolin-storage-local").LocalStorageProvider;
    try {
      ...
      throw new Error(
        `pangolin-worker: failed to load LocalStorageProvider: ${(err as Error).message}`,
      );
    }
  ...
  throw new Error(
    `pangolin-worker: unrecognized PANGOLIN_STORAGE_URI scheme: ${storageUri}`,
  );
}
...
/**
 * Extract the `<name>` segment from an pangolin:// URI.
 *
 * Shape: `pangolin://<namespace>/<type>/<name>/<contentHash>`.
 * Returns an empty string when the URI is too short to contain a name —
 * callers that need stricter validation should use `parsePangolinUri` directly.
 */
function deriveNameFromUri(uri: string): string {
  const parts = uri.replace(/^pangolin:\/\//, "").split("/");
  return parts[2] ?? "";
}
