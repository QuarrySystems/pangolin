...
const MINIO = process.env.PANGOLIN_S3_ENDPOINT; // set when MinIO is running
...
d('AwsS3MailboxClient against MinIO', () => {
  it('put/get/list/delete round-trips under prefix', async () => {
    ...
    await client.send(new CreateBucketCommand({ Bucket: 'pangolin-data' })).catch(() => {});
    const mb = new AwsS3MailboxClient({ client, bucket: 'pangolin-data', prefix: 'mailbox/' });
