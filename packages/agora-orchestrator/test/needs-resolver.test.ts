...
describe('selectProductRef', () => {
  it('returns resultRef for kind=patch', () => {
    const upstream = { ...base, id: 'a', resultRef: 'agora://ns/artifact/d/sha256:aa' } as ItemState;
    expect(selectProductRef(upstream, { kind: 'patch' })).toBe('agora://ns/artifact/d/sha256:aa');
  });
  ...
  it('returns the outputRef entry for kind=output with a matching path', () => {
    const upstream = { ...base, id: 'a', outputRefs: { 'dist/index.js': 'agora://ns/artifact/d/sha256:bb' } } as ItemState;
    expect(selectProductRef(upstream, { kind: 'output', path: 'dist/index.js' })).toBe('agora://ns/artifact/d/sha256:bb');
  });
...
describe('resolveInputRefs', () => {
  it('resolves a patch binding to the upstream resultRef', () => {
    const a = { ...base, id: 'a', resultRef: 'agora://ns/artifact/d/sha256:aa' } as ItemState;
  ...
  it('resolves an output-path binding to the upstream outputRef entry', () => {
    const a = { ...base, id: 'a', outputRefs: { 'dist/index.js': 'agora://ns/artifact/d/sha256:cc' } } as ItemState;
    ...
    expect(resolveInputRefs(b, new Map([['a', a]])))
      .toEqual({ inputRefs: { bundle: 'agora://ns/artifact/d/sha256:cc' } });
  });
