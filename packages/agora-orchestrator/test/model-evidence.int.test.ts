...
//     real (AGORA_MODEL → cfg.model → BlockContext → RuntimeInvocation.model),
...
import { buildAgoraUri, buildDispatchRecordUri } from '@quarry-systems/agora-core';
...
function makeMinimalClient(storage: LocalStorageProvider, namespace: string): AgoraClient {
...
describe('model evidence end-to-end (InprocWorkerExecutor)', () => {
  ...
  async function setupRun(opts: { subagentName: string; pipelineId: string; model?: string }) {
    ...
    const subagentPinnedUri = buildAgoraUri({
      namespace,
      type: 'subagent',
      name: subagentRef.name,
      contentHash: subagentRef.contentHash,
    });
    const pinnedPipelineUri = buildAgoraUri({
      namespace,
      type: 'pipeline',
  ...
  it('seals requested model in the manifest and actual usage in the sentinel on a real in-proc run', async () => {
    ...
    // (AGORA_MODEL → cfg.model wins at the BlockContext build site).
