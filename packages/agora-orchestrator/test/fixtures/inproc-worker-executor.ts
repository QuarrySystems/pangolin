...
//
// An Executor that runs agora-worker's runWorker() IN-PROCESS rather than in
...
import type { StorageProvider, RuntimeAdapter } from '@quarry-systems/agora-core';
import { runWorker } from '@quarry-systems/agora-worker';
...
export class InprocWorkerExecutor implements Executor {
  ...
  async fire(
    ...
    // Build inputRefs for AGORA_BUNDLE_REFS_JSON (if any).
    ...
    // Construct AGORA_BUNDLE_REFS_JSON. The worker expects:
    ...
    const workerEnv: Record<string, string> = {
      ...
      AGORA_STORAGE_URI: storageUri,
      AGORA_BUNDLE_REFS_JSON: JSON.stringify(bundleRefs),
      AGORA_INPUT_JSON: JSON.stringify(
