...
2. The `serve` driver ticks the PangolinOrchestrator until all items are terminal.
...
### CLI flow (when `pangolin` CLI is available)

```sh
pangolin orch serve &
pangolin orch submit plan.json
pangolin orch watch <runId>
pangolin orch audit <runId>
```
...
- A real `PangolinOrchestrator` with the fake executor drives the plan to completion.
