...
template: splash
hero:
  tagline: Submit a plan of agent tasks — they run in dependency order and fan out safely under file-locks. Get back reviewable patches and a verifiable audit trail — tamper-detecting by default, tamper-evident at the S3 Object Lock tier.
  actions:
...
<CardGrid>
  ...
  <Card title="Orchestrate a task plan" icon="puzzle">
    Run many agent tasks unattended with `agora orch`: tasks execute in dependency order, fan out in parallel under file-locks, and each drops a reviewable patch — with a verifiable audit bundle for the whole run.
