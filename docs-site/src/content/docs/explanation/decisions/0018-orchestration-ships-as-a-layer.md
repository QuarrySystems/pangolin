---
title: "ADR-0018: agora ships orchestration as a separate opt-in layer"
description: "agora ships orchestration as a distinct package + service (agora-orchestrator / offload), superseding ADR-0010's posture that orchestration lives only above agora and is out of scope forever. The client-SDK workflow-primitive rejection still stands."
status: accepted
date: 2026-06-01
deciders: agora-offload-v1
supersedes: ADR-0010
---

:::note[Relationship to ADR-0010]
This ADR **partially supersedes [ADR-0010](/agora/explanation/decisions/0010-no-workflow-primitive/)**.
ADR-0010's *narrow* decision — no `agora.workflow()` / `agora.procedure()` sugar
primitive on the client SDK — **still holds**. What this ADR reverses is ADR-0010's
*broad posture*: that orchestration "sits above agora, not inside it" and is "out of
scope forever." agora now ships orchestration as a distinct, opt-in layer.
:::

## Context

[ADR-0010](/agora/explanation/decisions/0010-no-workflow-primitive/) (MVP, 2026-05-21)
made two claims that have since diverged:

1. **Narrow:** agora should not ship an `agora.workflow()` / `agora.procedure()`
   primitive — a named, pre-composed dispatch template as sugar on the client SDK.
   Integrators wrap `dispatch()` themselves.
2. **Broad:** the architectural posture is that *"orchestration sits above agora,
...
- **A non-blocking execution model.** `AgoraClient.dispatch()` blocks on
...
**agora ships orchestration as a separate, opt-in layer — not as a primitive on
`AgoraClient`.** Concretely (per the
[Orchestrator architecture spec](https://github.com/QuarrySystems/agora/blob/main/docs/superpowers/specs/2026-05-28-agora-orchestrator-design.md)
and the
[Offload V1 delivery spec](https://github.com/QuarrySystems/agora/blob/main/docs/superpowers/specs/2026-05-29-agora-offload-v1-design.md),
shipped as agora-offload V1):

- Orchestration lives in its **own package** `@quarry-systems/agora-orchestrator`,
  with its seams + types in `src/contracts/` — **not** in `agora-core`, which stays
  minimal (D11). Low-level providers and the worker remain ignorant of orchestration.
- It runs as a **distinct long-running service** (`agora orch serve`) that is the
  exclusive owner of its run-state DB (D3). The CLI and MCP are *clients* of that
  service; they never open the DB.
- Its surface is **`agora orch` (submit / status / watch / cancel / audit)** plus
  **three client MCP tools** (`agora_orchestrator_submit`, `_status`, `_watch`).
...
- The MCP run-time surface is now **nine tools**, not six — the three
  `agora_orchestrator_*` client tools were added. ADR-0010's "stays at six tools"
...
- "Orchestration above agora, not inside it" no longer describes the codebase.
  The accurate statement is: orchestration is a **separable layer that ships in the
  agora repo as its own package and process**, composed at the deployment boundary
...
- The deferred layers (additional executors, packs, `Intent`/interpreter, named
  ...
  [Roadmap](/agora/explanation/project-status-roadmap/).
- agora is still **not a general-purpose workflow engine.** The orchestrator is
...
Per the [decision-records convention](/agora/explanation/decisions/), an ADR is
