...
| **Identity theft** — the agent inherits the worker's cloud creds and impersonates it to reach other tenants' data | A prompt-injected agent runs `env`/`printenv` and finds the worker's AWS chain or callback key | The worker→runtime **env firewall is default-DENY** (since [PR #71](https://github.com/quarrysystems/pangolin/pull/71)): only an allow-list of non-credential system vars crosses into the agent; every `PANGOLIN_*` var and the whole AWS credential chain are dropped **from the environment handed to the agent process**. Creds the agent genuinely needs arrive separately via the scoped secret lane. See [credentials](#credentials). | **The worker's own environment block is no longer readable.** This row previously recorded the opposite, and it was true when written: at a shared uid an agent read the worker's credentials straight out of `/proc/<pid>/environ`, verified 2026-07-23. The container entrypoint now `exec`s the worker with a cleared environment and hands the real one over an already-unlinked fd, which the worker restores into its heap — a region `/proc` does not expose, because that block is fixed at `execve`. No process in the namespace carries the credential, **verified per-commit by the `proc-exposure` workflow**, whose control arm must find a planted credential in the same run or the run is void. **Remaining limit:** the credential is resident in the worker's heap, so a host whose `ptrace_scope` permits same-uid tracing exposes it by that route — an ambient setting Pangolin does not control. Separately, an operator can widen the filter via `PANGOLIN_RUNTIME_ENV_ALLOW`, but **since [PR #141](https://github.com/quarrysystems/pangolin/pull/141) a bare `*` no longer re-opens everything**: a non-overridable hard-deny set is consulted *before* the operator allow-list, so the AWS credential chain (including the whole `AWS_CONTAINER_*` family) and the `PANGOLIN_CALLBACK_*_REF` / `PANGOLIN_PER_DISPATCH_SECRET_REFS_JSON` pointers cannot be passed by any allow-list entry. Non-credential vars such as `AWS_REGION` are deliberately still passable. |
...
| **Over-broad environment** — a worker infra cred is handed to the agent | Naive "pass the whole env through" | Default-deny env firewall (above). **Asymmetry to know:** allow-listed pass-through vars are *not* added to the redaction set — so **never allow-list a credential**; route every credential through the scoped, redacted secret lane. | The firewall scopes the environment handed to the agent; the worker's own process is covered separately by the entrypoint hand-off described in *Identity theft* above, so `/proc/<pid>/environ` no longer carries a credential for any process. **The credential-shaped half of the footgun is now prevented rather than documented** ([PR #141](https://github.com/quarrysystems/pangolin/pull/141)): a hard-deny set consulted before the allow-list makes the AWS chain and the callback/secret-ref pointers unreachable by any `PANGOLIN_RUNTIME_ENV_ALLOW` entry. The redaction asymmetry itself is unchanged and still documented-not-prevented — an operator who allow-lists some *other* credential-bearing var still gets it un-redacted, so the rule stands: route credentials through the secret lane. |
