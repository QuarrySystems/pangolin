...
6. Checks **provenance closure** of the typed-product handoff — every
   `manifests[*].inputRefs` value must be a product (`resultRef` or an `outputRefs`
   value) of a **completed** producer, derived from the *tamper-anchored chain*
   (the sealed `item.reconciled` entries), never the untrusted `items[]` export
   rows. A dangling or forged-producer ref fails as `handoff`.
7. Checks **manifest integrity** — binds every authorization-bearing manifest to a
   chained `item.fired` ref bidirectionally, purely by content hash, so a
   rewritten, downgraded, or appended-orphan manifest fails as `manifest`. (This
   also gates the orthogonal `authzTier` dimension.)

Any of the structural failures (1–4, 6, 7) makes the report `intact: false`, and —
this is the crux — the report's `claim` collapses to `tamper-detecting` regardless of
