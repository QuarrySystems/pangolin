...
>
> — [`2026-05-29-pangolin-offload-v1-design.md`](https://github.com/quarrysystems/pangolin/blob/main/docs/superpowers/specs/2026-05-29-agora-offload-v1-design.md) (status header)
...
3. **Secrets must be pre-declared in the task definition's `secrets:[]`.** The
   ...
   cannot inject new secrets at launch. Declare each secret (e.g. your Claude
   credential — `ANTHROPIC_API_KEY` or a `CLAUDE_CODE_OAUTH_TOKEN` subscription
   token — sourced from AWS Secrets Manager via the `AwsSecretStore` adapter) in
   the task definition so it is present when the task starts.
