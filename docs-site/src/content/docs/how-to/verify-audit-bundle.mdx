...
| `S3ObjectLockAnchor` | `external-immutable` | **`tamper-evident`** | The signed root lives in S3 Object Lock (compliance mode) — a separate trust domain that survives a DB-side tamper attempt. | It records *what ran* (environment + inputs by hash) and attests a **recorded dispatch-level authorization decision** (operator-self-asserted). It does **not** govern the agent's individual tool calls mid-run, does not reproduce the agent's *output*, and is not a compliance certification. |
...
An auditor handed a bundle usually does **not** want to install the orchestrator
...
It runs in one of **two modes**, and the mode sets the ceiling on the tamper claim
it can earn:

- **Offline (default — no `--anchor`).** It recomputes the chain and the Merkle root
  and compares against the **root embedded in the bundle**. With no external anchor
  to consult, the strongest claim it can reach is **`tamper-detecting`** — it proves
  the bundle is internally consistent, but not that the embedded root matches an
  immutable external copy.
- **Anchor-checked (`--anchor <verify-context.json>`).** The verify-context points
  the verifier at the **real WORM anchored root** (e.g. the S3 Object Lock object).
  The verifier fetches that external root and compares — exactly the check the
  embedded copy cannot do alone — so a passing run can reach the
...
see [Audit & guarantee tiers → Trusted time](/pangolin/explanation/audit-guarantee-tiers/#trusted-time--a-separate-dimension))
and the **`authzTier` field** (`recorded` — dispatch-level, operator-self-asserted; or `none` — not attested).
Note that `authzTier: 'recorded'` attests only that an authorization decision was recorded at the dispatch level (operator-self-asserted); it does **not** govern the agent's individual tool calls mid-run.
`--full` prints every ledger row.

<Aside type="note" title="The reimplementation spec">
Repo-root `VERIFICATION.md` is the auditor reimplementation spec: it documents the
bundle and verify-context formats, the verification algorithm, and the two modes
(offline → tamper-detecting ceiling; anchor-checked → tamper-evident ceiling). A
party who won't run *any* of our code can reimplement the verifier from it.
</Aside>
